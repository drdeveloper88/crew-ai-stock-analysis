...
```mermaid
graph TD
    A[User Input<br/>Stock Symbol(s)] --> B[Streamlit Web App<br/>app.py]
    B --> C[kickoff_stock Function<br/>crew.py]
    C --> D{CrewAI Crew<br/>with Agents & Tasks}
    
    D --> E[Financial Market Analyst Agent<br/>analyst_agent.py]
    D --> F[Trading Advisor Agent<br/>trader_agent.py]
    
    E --> G[Stock Research Tool<br/>stock_research_tool.py]
    F --> G
    
    G --> H[Yahoo Finance API<br/>Real-time Data]
    
    E --> I[Stock Analysis Task<br/>analyse_task.py]
    F --> J[Trading Decision Task<br/>trade_task.py]
    E --> K[Future Prediction Task<br/>prediction_task.py]
    
    I --> L[Analysis Results]
    J --> L
    K --> L
    
    L --> M[Display Results<br/>in Streamlit UI]
    
    C --> N{Fallback Mechanisms}
    N --> O[Reduced Token Models]
    N --> P[Rule-based Fallback<br/>No AI Inference]
